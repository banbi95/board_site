...
class HomeTest(TestCase):
    ...
    def test_home_contains_link_to_board_topics_page(self):
...
class BoardTopicsTest(TestCase):
    ...
    def test_board_topic_view_contains_navagation_links(self):
        board_topics_url = reverse('board:board_topics', kwargs={'pk': 1})
...
class NewTopicTest(TestCase):
    def setUp(self):
        Board.objects.create(name='Django', description='Django board.')
        User.objects.create_user(username='john', email='john@fox.com', password='123')
    ...
    def test_contains_form(self):  # <- new test
        url = reverse('board:new-topic', kwargs={'pk': 1})
        response = self.client.get(url)
